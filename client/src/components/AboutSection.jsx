...
const AboutSection = () => {
  return (
    <section id="about" className="py-20 bg-background max-w-full overflow-x-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {awards.map((award, index) => (
              <Card key={index} className="flex items-center p-4 travel-shadow hover:elevated-shadow smooth-transition max-w-full">
                <Award className="h-6 w-6 text-primary mr-3 flex-shrink-0" />
                <span className="text-muted-foreground text-sm truncate">{award}</span>
              </Card>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {stats.map((stat, index) => (
            <Card key={index} className="text-center travel-shadow hover:elevated-shadow smooth-transition max-w-full">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service, index) => (
              <Card key={index} className="travel-shadow hover:elevated-shadow smooth-transition border-0 h-full max-w-full">
                <CardContent className="p-6 flex flex-col h-full">
                  <div className="flex items-center space-x-4 mb-4">
                    <div className="p-3 bg-gradient-package rounded-lg flex-shrink-0">
                      <service.icon className="h-6 w-6 text-accent" />
                    </div>
                    <h4 className="font-semibold text-accent truncate">{service.title}</h4>
                  </div>
