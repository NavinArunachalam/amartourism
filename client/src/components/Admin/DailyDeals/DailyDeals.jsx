...
const DailyDeal = () => {
  ...
  // Fetch tour packages and filter daily deals
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Client-side filter to show only dailyDeal: true, limited to 4
        const dailyDeals = response.data.filter(p => p.dailyDeal === true).slice(0, 4);
        setTourPackages(dailyDeals);
        if (dailyDeals.length === 0) {
          setError("No daily deal packages available.");
          Toastify({
            text: "No daily deal packages available. Try adding a new one.",
            backgroundColor: "orange",
            ...toastConfig,
          }).showToast();
        }
      } catch (err) {
        console.error("Error fetching daily deal packages:", err);
        const errorMessage = err.response?.data?.message || "Failed to fetch daily deal packages.";
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex items-center justify-center">
      <div className="text-lg font-semibold text-gray-700 animate-pulse">Loading daily deal packages...</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <h1 className="text-3xl font-extrabold text-gray-900">Daily Deal Packages</h1>
          <div className="flex gap-3">
            {tourPackages.length < 4 && (
              <Link
                to="/admin/daily-deal/add"
              ...
              >
                <i className="fas fa-plus"></i> Add Daily Deal
              </Link>
            )}
            <Link
              to="/admin"
        ...
        {filteredTourPackages.length > 0 ? (
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        ...
                        <Link
                          to={`/admin/daily-deal/edit/${tourPackage._id}`}
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500 text-base font-medium">{error || "No daily deal packages found"}</p>
          </div>
        )}
      </div>
    </div>
